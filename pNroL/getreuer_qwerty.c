...
const uint16_t c_x_combo[] PROGMEM = {KC_C, KC_X, COMBO_END};
const uint16_t c_v_combo[] PROGMEM = {KC_C, NUM_V, COMBO_END};
const uint16_t f_g_combo[] PROGMEM = {LCTL_F, KC_G, COMBO_END};
// clang-format off
combo_t key_combos[] = {
    COMBO(caps_combo, CW_TOGG),          // C and , => activate Caps Word.
    COMBO(c_x_combo, KC_BSLS),           // C and X => backslash
    COMBO(c_v_combo, OSL(NUM)),          // C and V => one-shot NUM layer
    COMBO(f_g_combo, OSL(FUN)),          // F and G => one-shot FUN layer
};
// clang-format on
...
uint16_t get_alt_repeat_key_keycode_user(uint16_t keycode, uint8_t mods) {
  if ((mods & MOD_MASK_GUI)) {
    switch (keycode) {
      case SYM_A: return G(KC_C);  // Cmd+A -> Cmd+C
      case KC_C: return G(KC_V);    // Cmd+C -> Cmd+V
    }
    ...
    switch (keycode) {
      ...
      case G(KC_A): return G(KC_C);  // Cmd+A -> Cmd+C
    }
  }
  return KC_TRNS;
}
  ...
  if (record->event.pressed) {
    switch (keycode) {
      ...
      case SRCHSEL:  // Searches the current selection in a new tab.
        ...
        SEND_STRING_DELAY(
            SS_LGUI("ct") SS_DELAY(100) SS_LGUI("v") SS_TAP(X_ENTER),
            TAP_CODE_DELAY);
        return false;
